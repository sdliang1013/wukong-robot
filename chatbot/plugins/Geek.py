...
from chatbot.robot.sdk.AbstractPlugin import AbstractPlugin
    ...
    def handle(self, text, parsed):
        if any(word in text for word in ["开启", "激活", "开始", "进入", "打开"]):
            ...
            self.say(
                "进入极客模式",
                cache=True,
                onCompleted=lambda: self.onAsk(self.active_listen(silent=True)),
            )
        else:
    ...
    def onAsk(self, input):
        if input:
            ...
            if self.silent_count >= config.get("/geek/max_silent_count", 20):
                ...
                self.onAsk(self.active_listen(silent=True))

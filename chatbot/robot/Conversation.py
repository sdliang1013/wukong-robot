...
from chatbot.robot.Brain import Brain
from chatbot.robot.LifeCycleHandler import LifeCycleEvent
from chatbot.robot.Scheduler import Scheduler
from chatbot.robot.Sender import StatusData, ACTION_ROBOT_LISTEN, ACTION_USER_SPEAK, ACTION_ROBOT_THINK, \
    ACTION_ROBOT_SPEAK, STAGE_UNDERSTAND, STAGE_SEARCH, WebSocketSender
from chatbot.robot.compt import StreamStr
from chatbot.robot.sdk import History
from chatbot.snowboy import snowboydecoder
...
class Conversation(object):
    def __init__(self, life_cycle_event: LifeCycleEvent, profiling=False,
    ...
    def say_simple(self, msg, cache=False, plugin=None, resp_uuid=None, onCompleted=None, ):
        """
        说一句话
        :param msg: 内容
        ...
        self.speaker.speak_simple(msg=msg, req_id=resp_uuid, cache=cache,
                                  on_completed=onCompleted, )

    def play(self, src, delete=False, onCompleted=None, volume=1, interrupt=False):
        """播放一个音频"""
        self.speaker.play_audio(src=src, delete=delete, onCompleted=onCompleted, interrupt=interrupt)
    ...
    def converse(self, voice, callback=None, onSay=None):
        """核心对话逻辑"""
        logger.info("结束录音")
        self.life_cycle_event.fire_event("think")
        if self.profiling:
            ...
            pr = cProfile.Profile()
            pr.enable()
            self.do_converse(voice=voice, callback=callback, onSay=onSay)
            pr.disable()
            s = io.StringIO()
            sortby = "cumulative"
    ...
    def feedback(self, chat_id: str, data_id: str, useful: bool = True):
        """反馈"""
        return self.ai.feedback(chat_id=chat_id, data_id=data_id,
                                is_good=useful, opinion=None if useful else 'useless')

    def in_break_time(self, limit_time) -> bool:
        """在限定打断时间内"""
        return self.manual_break_time and (time.time() - self.manual_break_time < limit_time)
    ...
    def _response_gpt(self, query, resp_uuid):
        if self.ai and self.ai.support_stream():
            self.sender.put_message(action=ACTION_ROBOT_THINK,
                                    ...
                                    message="开始查找资料")
            stream = self.ai.stream_chat(texts=query, chat_id=resp_uuid, response_id=resp_uuid)
            self.sender.put_message(action=ACTION_ROBOT_THINK,
                                    data=StatusData(stage=STAGE_SEARCH, end=True).dict(),
                                    message="查找资料结束")
            self._stream_say(resp_uuid=resp_uuid, stream=stream,
                             cache=True, on_completed=self.check_restore)
        else:
            ...
            self.sender.put_message(action=ACTION_ROBOT_THINK,
                                    data=StatusData(stage=STAGE_SEARCH, end=True).dict(),
                                    message="查找资料结束")
            self._say(resp_uuid=resp_uuid, msg=msg, cache=True,
                      onCompleted=self.check_restore, with_interrupt=True)
    ...
    def _stream_say(self, resp_uuid, stream, cache=False, on_completed=None):
        ...
        stream_text = StreamStr(re_full=re_out["full"], re_pair=re_out["pair"], re_special=re_out["special"])
        stream_tts = StreamStr(re_full=re_tts["full"], re_pair=re_tts["pair"], re_special=re_tts["special"])
        ...
        try:
            for data in stream():
                ...
                if self.on_stream:
                    ...
                    if out_next:
                        self.on_stream(message=out_next, resp_uuid=resp_uuid, data=dict(end=False))
                ...
                for line in lines:
                    ...
                    if self.interrupted.is_set():
                        logger.info("响应已经被中断....")
                        return
                    audio = self.speaker.speak_in_order(line=line, req_id=resp_uuid,
                                                        index=index, cache=cache)
                    index += 1
                    if audio:
                        audios.append(audio)
            # 播放剩余的内容
            audio = self.speaker.speak_in_order(line=stream_tts.get_left(), req_id=resp_uuid,
                                                index=index, cache=cache, is_final=True)
            if audio:
                audios.append(audio)
        finally:
    ...
    def _say(self, resp_uuid, msg, cache=False, plugin="",
             onCompleted=None, append_history=True, with_interrupt=False):
        """
        说一句话
        :param msg: 内容
        :param cache: 是否缓存这句话的音频
        ...
        """
        audios = self.speaker.speak(msg=msg, req_id=resp_uuid, cache=cache, plugin=plugin,
                                    on_completed=onCompleted, with_interrupt=with_interrupt)
        self._after_write(msg=msg, resp_uuid=resp_uuid, plugin=plugin,
    ...
    def _after_write(self, msg, resp_uuid=None, append_history=True, plugin=""):
        ...
        # 结束事件
        self.life_cycle_event.fire_event(event="resp_end", text=msg, resp_uuid=resp_uuid)
        # 历史记录
        if append_history:
...
class OrderSpeaker:
    def __init__(self, life_cycle_event: LifeCycleEvent, sender: WebSocketSender = None):
        self.life_cycle_event = life_cycle_event
        self.sender = sender
        self.interrupted = threading.Event()  # 中断标记
        self.server_host = f"http://{config.get('/server/host')}:{config.get('/server/port')}"
        # 播放器
    ...
    def re_init(self):
        ...
        try:
            ...
            if self.dh_enabled:
                self.dh = DigitalHuman.get_engine_by_slug(config.get("/dh_engine/provider", "tencent-dh"))
        except:
            logger.critical("Speaker初始化失败.", exc_info=True)

    def speak(self, msg, req_id=None, cache=False, plugin="", on_completed=None, with_interrupt=False) -> list:
        """
        说长句子
        :param msg: 内容
        :param cache: 是否缓存这句话的音频
        ...
        # 重置index
        if self.dh:
            self._dhs(lines=lines, on_completed=on_completed, with_interrupt=with_interrupt,
                      req_id=req_id)
        else:
            audios = self._tts(lines=lines, on_completed=on_completed, with_interrupt=with_interrupt,
                               cache=cache, )
        return audios
    ...
    def speak_simple(self, msg, req_id=None, cache=True, on_completed=None):
        ...
        def _play_voice(audio):
            self.play_audio(src=audio, delete=not cache, onCompleted=on_completed, interrupt=True)
    ...
    def speak_in_order(self, line, req_id=None, index=0, cache=True, on_completed=None, is_final=False) -> str:
        """
        结合start_order和end_order使用
        """
        if self.dh:
            self._dh_in_order(msg=line, req_id=req_id, index=index + 1, is_final=is_final, on_completed=on_completed)
        else:
            return self._tts_in_order(msg=line, cache=cache, index=index, on_completed=on_completed)
    ...
    def play_audio(self, src, delete=False, onCompleted=None, interrupt=False):
        ...
        if interrupt:
            ...
            # 清除中断标记
            self.interrupted.clear()
        Player.play(fname=src, delete=delete, onCompleted=onCompleted, )
    ...
    def begin_order(self, notify=True):
        ...
        # 发送消息: 机器人开始说话
        if notify:
            self.sender.put_message(action=ACTION_ROBOT_SPEAK,
    ...
    def audio_path(self, audios) -> Union[str, list]:
        ...
        return [f"{self.server_host}/audio/{os.path.basename(voice)}" for voice in audios]
                ...
                with ThreadPoolExecutor(max_workers=3) as pool:
                    ...
                    for line in lines:
                        ...
                        if with_interrupt and self.interrupted.is_set():
                            ...
                            break
                        task = pool.submit(self._tts_in_order,
                                           msg=line.strip(), cache=cache,
                                           index=index, on_completed=None)
                        index += 1

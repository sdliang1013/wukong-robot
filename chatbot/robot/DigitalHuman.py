...
from chatbot.robot import config, log, schedulers
...
class TecentDigitalHuman(AbstractDigitalHuman):
    ...
    def __init__(self, base_url, wss_url, access_token, app_key, project_id, user_id, **kwargs):
    ...
    def CreateSession(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info("SessionStatus: %s,PlayStreamAddr: %s",
                            self.session_status, self.play_stream_addr)
                return responseData
            else:
    ...
    def ListSessionOfProjectId(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                code = responseData['Header']['Code']
                if code != 0:
                    logger.error(f"查询数智人项目下的会话列表失败，response: {response}")
                return responseData['Payload']['Sessions']
            else:
                logger.error(f"{self.SLUG} 查询数智人项目下的会话列表出错了: {response.text}")
                return response.text
        except Exception as e:
    ...
    def interrupt(self, req_id=None):
        ...
        self.send_cmd(cmd="SEND_STREAMTEXT", data={"Interrupt": True, "Seq": 1}, req_id=req_id)
    ...
    def GenSignature(self, signing_content):
        # 计算 HMAC-SHA256 值
        h = hmac.new(self.access_token.encode(), signing_content.encode(), hashlib.sha256)

...
from chatbot.robot import config, constants, log
from chatbot.robot.compt import ThreadManager
from chatbot.robot.sdk.VolcengineSpeech import StreamLmClient
...
class FunRTAsr(AbstractRTAsr):
    ...
    def connect(self, on_message,
                on_open=None, on_error=None, on_close=None, **kwargs) -> any:
    ...
    def run(self, conn: websocket.WebSocketApp, **kwargs):
        conn.run_forever(ping_interval=20,
    ...
    def send_meta(self, conn: websocket.WebSocketApp, data: Union[str, dict] = None,
                  is_speaking=True, **kwargs):
        """
        is_speaking 表示断句尾点,例如,vad切割点,或者一条wav结束
        """
        ops_data = {"mode": self.mode,
    ...
    def send_voice(self, conn: websocket.WebSocketApp, data: Union[str, bytes], **kwargs):
    ...
    def wrap_message(self, on_message):
        def rev_message(ws, message, *args, **kwargs):
            ...
            # 封装成统一格式
            on_message(AsrResponse(text=data.get('text', None),
                                   is_end=data["mode"] == "2pass-offline"))
    ...
    def _load_hotwords(self, hot_words) -> str:
        ...
        if isinstance(hot_words, str):
            with open(file=constants.getConfigData(hot_words), mode="r", encoding='utf8') as f:
        ...
        for line in hot_words:
            ...
            words = line.split(" ")
            if len(words) != 2:
                logger.warning("Please checkout format of hot words: <word> <hot_level>")
                continue
            try:
...
class VolcengineRTAsr(AbstractRTAsr):
    ...
    def __init__(self, mode=None, chunk_time=None, **kwargs):
        ...
        if hot_words:
            self.hot_words = self._load_hotwords(hot_words)
        self.vol_engine = StreamLmClient(app_id=self.app_id, token=self.token,
                                         format="pcm", hot_words=self.hot_words)
    ...
    def wrap_message(self, on_message):
        def rev_message(ws, message, *args, **kwargs):
            ...
            # 封装成统一格式
            on_message(AsrResponse(text=data.get('payload_msg', {}).get('result', {}).get('text', None),
...
class MixedConn:
    ...
    def __init__(self, conn_fun: websocket.WebSocketApp, conn_volcano: websocket.WebSocketApp):
...
class MixedRTAsr(AbstractRTAsr):
    ...
    def connect(self, on_message,
        ...
        return MixedConn(conn_fun=self.asr_fun.connect(on_message=func_msg),
                         conn_volcano=self.asr_volcano.connect(
                             on_message=func_msg, on_open=on_open,
                             on_error=on_error, on_close=on_close, **kwargs))

    def run(self, conn: MixedConn, **kwargs):
        # funasr
        t = ThreadManager.new(target=self.asr_fun.run,
                              kwargs=dict(conn=conn.conn_fun, **kwargs))
        t.start()
        # volcengine
    ...
    def send_meta(self, conn: MixedConn, data: Union[str, dict] = None,
                  is_speaking=True, **kwargs):
        """
        is_speaking 表示断句尾点,例如,vad切割点,或者一条wav结束
        """
        try:
            self.asr_fun.send_meta(conn=conn.conn_fun, data=data, is_speaking=is_speaking, **kwargs)
        finally:
            self.asr_volcano.send_meta(conn=conn.conn_volcano, data=data, is_speaking=is_speaking, **kwargs)
...
class RTAsrClient:
    ...
    def connect(self):
        ...
        self.running.set()
        # 连接ASR
        self.thread_asr = ThreadManager.new(target=self._run_asr_forever,
                                            kwargs=dict(on_message=self._on_message))
    ...
    def _init_asr(self):
        ...
        return get_rtasr_by_slug(slug=rt_engine,
...
def get_rtasr_by_slug(slug, **kwargs) -> AbstractRTAsr:
    ...
    selects = list(
        filter(
            lambda _cls: hasattr(_cls, "SLUG") and _cls.SLUG == slug, get_rtasrs()
        )
    )
    if len(selects) == 0:

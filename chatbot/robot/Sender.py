...
from chatbot.robot import log, config
...
class ExtWebSocketHandler(WebSocketHandler, RequestHandler):
    ...
    def validate(self, validation):
        ...
        return validation == config.get("/server/validate", "") or \
            validation == str(object=self.get_cookie("validation"))
    ...
    def send_response(self, resp_uuid, action: str = None,
                      data=None, message: str = None,
                      plugin="", user_id=None, t=None):
        resp = {
...
class WebSocketSender:
    ...
    def send_message(self, action: str, data: dict = None, message: str = None, resp_uuid=None,
                     **kwargs):
        logger.info("机器人状态：%s", message)
        resp_uuid = resp_uuid or uuid.uuid4().hex
        for _, client in enumerate(self.clients):
            client.send_response(resp_uuid=resp_uuid, action=action,
                                 data=data, message=message, **kwargs)

    def put_message(self, action: str, data: dict = None, message: str = None, resp_uuid=None,
                    **kwargs):

...
from chatbot.robot import config, log, RTAsr
from chatbot.robot.agent import get_agent_by_slug
from chatbot.robot.compt import Robot, ThreadManager, StateMachine
from chatbot.robot.detector import get_detector_by_slug
from chatbot.robot.enums import AssistantStatus, AssistantEvent
from chatbot.robot.recognizer import get_recongnizer_by_slug
...
class VoiceAssistant(Robot):
    ...
    def __init__(self, wukong, **kwargs) -> None:
        self.wukong = wukong
    ...
    def _init_components(self):
        ...
        self.recognizer = get_recongnizer_by_slug(slug=config.get("/recongnizer", "realtime"),
                                                  bot=self, asr=self.asr,
                                                  conversation=self.wukong.conversation,
                                                  sender=self.wukong.sender,)
        self.agent = get_agent_by_slug(slug=config.get("/agent", "conversation"),
                                       bot=self, conversation=self.wukong.conversation,)

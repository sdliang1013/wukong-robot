...
class StateMachine:
    ...
    def regedit(self, from_status: Enum, to_status: Enum, event: Enum, call=None, replace=False):
        key = self.get_key(status=from_status, event=event)
        if key in self.changes and not replace:
            raise RuntimeError(f"重复注册的状态和事件: status={from_status.value}, event={event.value}")
...
class VolumeControl:
    ...
    @classmethod
    def _get_volume_linux(cls, ) -> float:
        """系统音量（0-100）"""
        output = subprocess.check_output(["pactl", "get-sink-volume", cls.device]).decode()
...
class ThreadManager:
    ...
    @classmethod
    def new(cls, group=None, target=None, name=None,
            args=(), kwargs=None, *, daemon=None) -> threading.Thread:
        thread = threading.Thread(group=group, target=target, name=name,
                                  args=args, kwargs=kwargs, daemon=None)
    ...
    @classmethod
    def alives(cls, ) -> List[threading.Thread]:
        t_alive = list()

...
from chatbot.robot import config, log, utils
...
class ConfigJob(object):
    ...
    def __init__(self, scheduler: DeferredScheduler, id=None, name=None, **kwargs):
        ...
        if enabled:
            scheduler.add_job(id=id or uuid.uuid4().hex, name=name or self.SLUG,
                              func=self.run_job, **kwargs)
...
class ScreenControlJob(ConfigJob):
    ...
    def __init__(self, scheduler: DeferredScheduler, **kwargs):
        self.hour_start = datetime.time(hour=config.get(f"/jobs/{self.SLUG}/hour_start", 19))
        self.hour_end = datetime.time(hour=config.get(f"/jobs/{self.SLUG}/hour_end", 6))
        self.interval_minute = 10
        super().__init__(scheduler=scheduler, trigger="interval", minutes=self.interval_minute,
                         **kwargs)

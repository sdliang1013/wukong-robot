...
class StreamLmClient:
    def __init__(self, app_id, token, **kwargs):
        ...
        self.online_url = kwargs.get("online_url", "wss://openspeech.bytedance.com/api/v3/sauc/bigmodel")
        self.offline_url = kwargs.get("offline_url", "wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_nostream")

    def conn_online(self, on_message, on_open=None, on_close=None, on_error=None) -> WebSocketApp:
        return WebSocketApp(url=self.online_url, header=self.ws_header(str(uuid.uuid4())),
                            on_message=on_message,
                            on_open=on_open or self._on_open,
                            on_error=on_error or self._on_error,
                            on_close=on_close or self._on_close, )

    def conn_offline(self, on_message, on_open=None, on_close=None, on_error=None) -> WebSocketApp:
        return WebSocketApp(url=self.offline_url, header=self.ws_header(str(uuid.uuid4())),
                            on_message=on_message,
                            on_open=on_open or self._on_open,
                            on_error=on_error or self._on_error,
                            on_close=on_close or self._on_close, )
    ...
    def meta_payload(self):
        return {
            ...
            "request": {
                ...
                "context": {
                    "hotwords": [{"word": word} for word in self.hot_words]
                }
            }
        }
    ...
    def send_meta_info(self, ws: WebSocketApp, seq:int=None):
        ...
        # header
        meta_request = bytearray(generate_header(message_type_specific_flags=specific_flags))
        if seq:
            meta_request.extend(generate_before_payload(sequence=seq))
        # payload
        meta_request.extend((len(payload_bytes)).to_bytes(4, 'big'))  # payload size(4 bytes)
    ...
    def send_audio_data(self, ws: WebSocketApp, chunk_data: bytes, seq: int=None, is_final=False):
        specific_flags = REQ_FULL_OR_NO_FINAL
        if seq:
            specific_flags = REQ_SEQUENCE
        if seq and is_final:
        ...
        # payload
        audio_request.extend((len(payload_bytes)).to_bytes(4, 'big'))  # payload size(4 bytes)
    ...
    def chunk_time(self, last_time: float) -> float:
        """音频块时长"""
        return max(.0, (self.seg_duration / 1000.0 - (time.time() - last_time)))
    ...
    def _on_close(self, ws: WebSocketApp, code, message):
        logger.info("volcengine ws closed, close_status_code={} close_msg={}".format(code, message))

...
from chatbot.robot import config, log
from chatbot.robot.Sender import ACTION_USER_SPEAK
from chatbot.robot.enums import AssistantEvent, AssistantStatus
from chatbot.schemas.core import Response
from chatbot.server.core import BaseHandler, ApiBaseHandler
from chatbot.srv.navigation import OfficeService, DoctorService, FaqService
...
class ExtLoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        if data.get("username") == config.get(item="/server/username") and \
                pwd_sign == config.get(item="/server/validate"):
...
class DigitalHumanHandler(BaseHandler):
    ...
    def initialize(self, wukong=None, **kwargs):
        super(DigitalHumanHandler, self).initialize(**kwargs)
        self.dh = wukong.conversation.speaker.dh
...
class DetectorHandler(BaseHandler):
    ...
    def start_log(self):
        self.wukong.robot.open_log()
        self.response(Response.ok())

    def stop_log(self):
        self.wukong.robot.close_log()
        self.response(Response.ok())
...
class HandControlHandler(BaseHandler):
    ...
    def wakeup_robot(self):
        # 手工唤醒
        self.wukong.robot.action(AssistantEvent.CTRL_WAKEUP, manual=True)
        self.response(Response.ok())

    def interrupt_robot(self):
        # 打断说话
        self.wukong.robot.action(AssistantEvent.CTRL_STOP_RESP, manual=True)
        self.response(Response.ok())

    def interrupt_and_wakeup(self):
        # 重新提问
        self.wukong.robot.action(AssistantEvent.CTRL_ASK_AGAIN,
                                 interrupt_time=self.interrupt_time, manual=True)
        self.response(Response.ok())

    def sleep_robot(self):
        # 打断说话
        if not self.wukong.robot.status() == AssistantStatus.DEFAULT:
            self.wukong.robot.action(AssistantEvent.CTRL_STOP_RESP, manual=True)
        # 反馈休眠
        if self.wukong and hasattr(self.wukong, 'life_cycle_event'):
            self.wukong.life_cycle_event.fire_event(event="sleep")
        self.response(Response.ok())

    def commit_query(self):
        """直接提交"""
        self.wukong.robot.action(AssistantEvent.CTRL_COMMIT_LISTEN)
        self.response(Response.ok())
...
class TuningControlHandler(BaseHandler):
    ...
    def feedback(self):
        ...
        if self.wukong and hasattr(self.wukong, 'conversation'):
            resp = self.wukong.conversation.feedback(chat_id=data_id,
                                                     data_id=data_id,
                                                     useful=useful)
...
class ChatApiHandler(ApiBaseHandler):
    ...
    def post_send_query(self) -> Response:
        ...
        if not query:
            ...
            self.wukong.sender.put_message(action=ACTION_USER_SPEAK,
                                           data={"end": True},
                                           message=query,
                                           t=0)
            # 另起线程执行
            self.wukong.robot.action(AssistantEvent.CTRL_QUERY,
                                     query=query, req_uuid=req_uuid)
            return Response.ok()

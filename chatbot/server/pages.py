...
from chatbot.robot.sdk.History import History
from chatbot.server.apis import DigitalHumanHandler
from chatbot.server.core import BaseHandler
...
class MainHandler(BaseHandler):
    def get(self):
        if not self.valid_to_login():
            return
        if self.wukong and self.wukong.conversation:
            # info = Updater.fetch()
            info = {}
        ...
        if self.validate(self.get_argument("validate", default=None)):
            if self.get_argument("type") == "text":
                ...
                if query == "":
                    ...
                    self.wukong.conversation.do_response(
                        query=query,
                        req_uuid=req_uuid,
                ...
                self.wukong.conversation.converse(
                    voice=nfile,
    ...
    def get(self):
        if not self.validate(self.get_argument("validate", default=None)):
            ...
            res = {
                "code": 0,
                "message": "ok",
                "history": json.dumps(self.wukong.conversation.getHistory().cache),
            }
            self.write(json.dumps(res))
        self.finish()
...
class DHPageHandler(BaseHandler):
    ...
    def get(self):
        if not self.valid_to_login():
            return
        self.render(template_name="digital-human.html",
                    **DigitalHumanHandler.info(dh=self.wukong.conversation.speaker.dh))

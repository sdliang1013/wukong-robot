...
from chatbot.robot import config, log, constants
from chatbot.robot.Sender import ExtWebSocketHandler
from chatbot.robot.compt import ThreadManager
from chatbot.server.apis import DigitalHumanHandler, ExtLoginHandler, ExtLogoutHandler, DetectorHandler, \
    HandControlHandler, TuningControlHandler, ChatApiHandler, NavigationHandler
from chatbot.server.core import api_base, Route, add_routes
from chatbot.server.pages import MainHandler, LoginHandler, LogoutHandler, GetHistoryHandler, ChatHandler, \
    MessageUpdatesHandler, ConfigHandler, ConfigPageHandler, OperateHandler, LogPageHandler, GetLogHandler, APIHandler, \
    QAHandler, UpdateHandler, DonateHandler, DHPageHandler
...
# 静态资源
route_statics = [
    Route(path=r"/photo/(.+\.(?:png|jpg|jpeg|bmp|gif|JPG|PNG|JPEG|BMP|GIF))",
          handler=tornado.web.StaticFileHandler,
          kwarg={"path": config.get("/camera/dest_path", os.path.join(constants.WWW_PATH, "static"))}),
    Route(path=r"/audio/(.+\.(?:mp3|wav|pcm))",
          handler=tornado.web.StaticFileHandler,
...
def start_server(wk):
    ...
    try:
        ...
        # 页面路由
        add_routes(app=application, routes=route_pages, kwarg=dict(wukong=wk))
        # 静态资源路由
        add_routes(app=application, routes=route_statics)
        # API路由
        add_routes(app=application, routes=route_apis, kwarg=dict(wukong=wk))
...
def run(wk, debug=False):
    settings["debug"] = debug
    t = ThreadManager.new(target=lambda: start_server(wk=wk))
    t.start()

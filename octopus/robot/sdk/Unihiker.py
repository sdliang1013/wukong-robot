...
class Unihiker(object):
    def __init__(self):
        ...
        self._gui.draw_image(w=240, h=320, image=constants.getRS("background.png"))
    ...
    def record(self, t, text=""):
        (
            self._my_bubble.config(text=text, color="black")
            if t == 0
            else self._bot_bubble.config(text=text)
        )
